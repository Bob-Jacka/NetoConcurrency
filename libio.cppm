...
export namespace concurrency {
    ...
    /**
     * Make current execution thread sleap
     * @param second_cound count of seconds to sleep
     */
//    template<std::chrono::milliseconds>
    void sleep(const int second_cound) {
        std::this_thread::sleep_for(std::chrono::seconds(second_cound));
    }
}
